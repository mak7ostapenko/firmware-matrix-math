...
public:
	...
	*/
	Euler(const Dcm<Type> &dcm)
	{
		...
		if ((std::fabs(theta() - static_cast<Type>(M_PI_PRECISE / 2))) < static_cast<Type>(1.0e-3)) {
			phi() = 0;
			psi() = std::atan2(dcm(1, 2), dcm(0, 2));

		} else if ((std::fabs(theta() + static_cast<Type>(M_PI_PRECISE / 2))) < static_cast<Type>(1.0e-3)) {
			phi() = 0;
			psi() = std::atan2(-dcm(1, 2), -dcm(0, 2));
